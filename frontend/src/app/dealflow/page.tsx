...
import { LayoutList, Plus, Search, Trash2 } from "lucide-react";
import { toast } from "sonner";

import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { useAuth } from "@/contexts/auth-context";
import { dealflowApi, type DealflowEntryCreateBody } from "@/lib/api";
            ...
            <tbody>
              {list.map((e) => (
                <tr key={e.id} className="border-b last:border-0 hover:bg-muted/30">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {e.promoted_company_id && (
                        <Link
                          ...
                          className="rounded bg-green-500/15 px-1.5 py-0.5 text-xs text-green-700 dark:text-green-400 hover:underline"
                        >
                          In Deal Room
                        </Link>
                      )}
                  ...
                  <td className="text-muted-foreground p-3 max-w-[200px] truncate" title={e.one_liner ?? undefined}>
                    {e.one_liner || "—"}
                  </td>
                  <td className="p-3">{e.stage || "—"}</td>
                  <td className="text-right p-3">{formatCurrency(e.amount_raising)}</td>
                  <td className="text-right p-3">{formatCurrency(e.valuation)}</td>
                  <td className="text-muted-foreground p-3">{e.location || "—"}</td>
                  <td className="p-3">{e.source_type || "—"}</td>
                  <td className="p-3">
                    <span className="rounded bg-muted px-1.5 py-0.5 text-xs">{e.status}</span>
                  </td>
